...
export const getAllCompanions = async ({ limit = 10, page = 1, subject, topic }: GetAllCompanions) => {
    ...
    const companionsWithBookmarkStatus = await Promise.all(
        companions.map(async (companion) => {
            const bookmarked = await isCompanionBookmarked(companion.id);
...
export const getCompanion = async (id: string) => {
    ...
    // Add bookmarked status to the companion
    const bookmarked = await isCompanionBookmarked(id);
    return { ...data[0], bookmarked };
}
...
export const addBookmark = async (companionId: string, path: string) => {
    ...
    const isBookmarked = await isCompanionBookmarked(companionId);
...
// It's almost the same as getUserCompanion, but it's for the bookmarked companions
